...
Escriba una función llamada `count_something` para trabajar con la tabla `gapminder`. `count_something` debe agrupar por una variable y sumar otra. Por ejemplo, `count_something(gapminder, continent, pop)` debería devolver lo siguiente:
...
count_something(gapminder, continent, pop)
...
Crea una función `plot_table` que acompañe a `count_something`. `plot_table` debe recibir la tabla creada por `count_something` y devolver un gráfico de barras. Es importante que la función reciba el nombre de la variable x y la variable y. Además, debe existir un parámetro para agregar un título al gráfico. El llamado a la función `plot_table(table, continent, n,  "Población en cada continente" )` debería tener este resultado 

.pull-left[

```{r, echo=F, fig.height=5, fig.width=4.5}

tabla <- count_something(gapminder, continent, pop)
...
plot_table(tabla, continent, n,  "Población en cada continente" )
...
plot_table(table, continent, n,  "Población en cada continente" )
...
```
Retomemos nuestras funciones `count_something` y `plot_table`. 
...
plots_by_year <- gapminder_list %>% 
  map(~count_something(.x, continent, pop)) %>% 
...
```{r, eval=F}
gapminder_list <- split(gapminder, gapminder$year)
plots <- list(vector(length = length(gapminder_list) ))
i <- 1
for (plot in gapminder_list) {
  table <- count_something(plot, continent, pop)
...
Modifica la función `plot_table` para que el año del gráfico aparezca en el subtítulo y no en el título. La función modificada debería recibir un parámetro extra, que permita agregar el año.


**Ejercicio 3 (solo para valientes)**

El siguiente *for* anidado genera pares de *x* e *y*. El ejercicio consiste en escribir el código con una sintaxis de `purrr`

Es posible que la sintaxis llegue a ser un poco confusa para este caso. La próxima semana reflexionaremos sobre la pertinencia de `purrr` para algunas tareas.  
